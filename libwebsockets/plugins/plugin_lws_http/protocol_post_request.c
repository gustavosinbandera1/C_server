...
#include <stdio.h>
 ...
 struct per_session_data__post_request {
	struct lws_spa *spa;
...
void CreateHeaders(struct lws *wsi,void *user,char* jsonText) {
	struct per_session_data__post_request *pss = (struct per_session_data__post_request *)user;
...
static int
callback_post_request(struct lws *wsi, enum lws_callback_reasons reason,
		   void *user, void *in, size_t len)
{
 	struct per_session_data__post_request *pss =
			(struct per_session_data__post_request *)user;
	unsigned char *p, *start, *end;
	...
	switch (reason) {
		...
		case LWS_CALLBACK_HTTP_BODY:
			fprintf(stderr,"CALLBACK HTTP BODY\n");
...
#define LWS_PLUGIN_PROTOCOL_POST_REQUEST \
	{ \
		"protocol-post-demo", \
		callback_post_request, \
		sizeof(struct per_session_data__post_request), \
		1024, \
		0, NULL, 0 \
	}
...
static const struct lws_protocols protocols[] = {
	LWS_PLUGIN_PROTOCOL_POST_REQUEST
};

LWS_EXTERN LWS_VISIBLE int
init_protocol_post_request(struct lws_context *context,
...
LWS_EXTERN LWS_VISIBLE int
destroy_protocol_post_request(struct lws_context *context)
{
	return 0;
}
